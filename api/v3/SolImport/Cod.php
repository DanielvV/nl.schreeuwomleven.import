<?php
/**
 * Doel verwerken van de column 'cod' vanuit de laad tabel
...
function civicrm_api3_sol_import_Cod($params) {
  set_time_limit(0);
  ...
  while ($daoSource->fetch()) {
    $codImport = new CRM_SolImport_CodImport($element, $daoSource, $logger);
    $result = $codImport->process();
    if ($result == FALSE) {
  ...
  if (empty($daoSource->N)) {
    $returnValues[] = 'Nothing to import - alle codes are done';
  } else {
  ...
  return civicrm_api3_create_success($returnValues, $params, 'ImportSol', 'Cod');
}
