<?php
/**
 * Doel verwerken van de column 'Emailadressen' vanuit de laad tabel
 ...
 */
function civicrm_api3_sol_import_Email($params) {
  set_time_limit(0);
  ...
  while ($daoSource->fetch()) {
    $emailImport = new CRM_SolImport_EmailImport($element, $daoSource, $logger);
    $result = $emailImport->process();
    if ($result == FALSE) {
  ...
  if (empty($daoSource->N)) {
    $returnValues[] = 'Nothing to import - alle email adresses are done';
  } else {
  ...
  return civicrm_api3_create_success($returnValues, $params, 'ImportSol', 'Email');
}
